...
public class ProductServiceImpl implements ProductService{
    ...
    public String[] qrPath() throws IOException, WriterException {
        ...
        Product product = productMapper.getProduct(productMapper.lastId());
        String text = "商品名:"+product.getName() + "\n价格:"+product.getPrice()+"\n生产日期:"+product.getDate()+"\n生产地:"+product.getPlace()+ "\n经销商:"+product.getDistributor();
        QRCodeGenerator.code(text, 350, 350, path);
        QRCodeGenerator.code(text, 350, 350, path3);
        sqlSession.close();
        String[] paths = {path, path2};
        return paths;
    }
     ...
     */
    @Override
    public void updateProduct(Product product) throws IOException, WriterException {
        SqlSession sqlSession = MybatisUtils.getSqlSession();
        ProductMapper productMapper = sqlSession.getMapper(ProductMapper.class);
        productMapper.updateProduct(product);
        sqlSession.commit();
        String path = "src/main/resources/static/images/image/qrcode/qrcode" + product.getId() + ".png";
        String path2 = "/images/image/qrcode/qrcode" + product.getId() + ".png";
        String path3 = "target/classes/static" + path2;
        String text = "商品名:"+product.getName() + "\n价格:"+product.getPrice()+"\n生产日期:"+product.getDate()+"\n生产地:"+product.getPlace()+ "\n经销商:"+product.getDistributor();
        QRCodeGenerator.code(text, 350, 350, path);
        QRCodeGenerator.code(text, 350, 350, path3);
